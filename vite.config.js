  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        if (req.url.startsWith('/api/daily')) return dailyHandler(req, res);
        if (req.url.startsWith('/api/search')) return searchHandler(req, res);
        if (req.url.startsWith('/api/audio')) return audioHandler(req, res);
        next();
      });
    },
  };
