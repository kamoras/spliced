...
- **Pinned catalog.** [`api/_catalog.json`](./api/_catalog.json) is a catalog of
  ...
  [`scripts/build-catalog.ts`](./scripts/build-catalog.ts): cross-era curated
...
The codebase is **TypeScript** throughout (app, serverless functions, and
build scripts), type-checked in CI with `npm run typecheck`.
