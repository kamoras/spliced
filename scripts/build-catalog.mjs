...
//   1. Curated cross-era classics (api/_songs.js), resolved via iTunes Search.
...
async function chartIds() {
  ...
  for (const genre of GENRES) {
    ...
    try {
      const feed = (await getJson(url)).feed.entry || [];
      for (const entry of feed) {
        const id = entry.id?.attributes?.['im:id'];
        if (id) ids.push(id);
      }
      process.stdout.write(`genre ${genre}: ${feed.length}  `);
    } catch (err) {
      console.warn(`genre ${genre} failed (${err.message})`);
    }
    await delay(120);
  }
  console.log('');
...
async function lookup(ids) {
  ...
  for (let i = 0; i < ids.length; i += 180) {
    ...
    try {
      for (const t of (await getJson(url)).results || []) {
        if (t.kind === 'song' && t.previewUrl && t.trackId)
          byId.set(t.trackId, t);
      }
    } catch (err) {
      console.warn(`lookup batch failed (${err.message})`);
    }
    await delay(120);
  }
  return byId;
